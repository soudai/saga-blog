...
- `app_collab`: スペース、テンプレート、添付、下書き、議事録アクション
...
### 5.6 スペース / ページ階層 / テンプレート

#### `spaces`

| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `space_id` | `uuid` | PK | スペース ID |
| `space_key` | `citext` | UNIQUE | URL 用識別子 |
| `created_by_user_id` | `uuid` | FK users | 作成者 |
| `created_at` | `timestamptz` | NOT NULL | 作成時刻 |
...
| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `space_revision_id` | `uuid` | PK | revision ID |
| `space_id` | `uuid` | FK spaces | 対象スペース |
| `revision_number` | `integer` | UNIQUE(space_id, revision_number) | revision |
| `name` | `text` | NOT NULL | 表示名 |
| `description` | `text` | NULL | 説明 |
| `visibility` | `text` | NOT NULL | `private` / `restricted` / `public` |
| `actor_user_id` | `uuid` | FK users | 更新者 |
| `created_at` | `timestamptz` | NOT NULL | 作成時刻 |
...
| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `space_membership_event_id` | `uuid` | PK | event ID |
| `space_id` | `uuid` | FK spaces | 対象スペース |
| `user_id` | `uuid` | FK users | 対象ユーザー |
| `role` | `text` | NOT NULL | `owner` / `editor` / `viewer` |
| `event_type` | `text` | NOT NULL | `member_added` / `member_removed` / `role_changed` |
| `actor_user_id` | `uuid` | FK users | 実行者 |
| `occurred_at` | `timestamptz` | NOT NULL | 発生時刻 |

#### `article_path_revisions`

| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `article_path_revision_id` | `uuid` | PK | revision ID |
| `article_id` | `uuid` | FK articles | 対象記事 |
| `revision_number` | `integer` | UNIQUE(article_id, revision_number) | revision |
| `space_id` | `uuid` | FK spaces | 所属スペース |
| `path` | `text` | NOT NULL | 例 `/db/adr/2026/decision-001` |
| `actor_user_id` | `uuid` | FK users | 実行者 |
| `created_at` | `timestamptz` | NOT NULL | 作成時刻 |
...
| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `article_path_redirect_id` | `uuid` | PK | redirect ID |
| `space_id` | `uuid` | FK spaces | 対象スペース |
| `from_path` | `text` | NOT NULL | 旧パス |
| `to_article_id` | `uuid` | FK articles | 移動先記事 |
| `created_at` | `timestamptz` | NOT NULL | 作成時刻 |
...
| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `template_id` | `uuid` | PK | テンプレート ID |
| `template_key` | `citext` | UNIQUE | 例 `daily-report`, `adr`, `meeting-minutes` |
| `name` | `text` | NOT NULL | 表示名 |
| `purpose_type` | `text` | NOT NULL | `knowledge` / `daily_report` / `design_doc` / `db_design` / `adr` / `meeting_minutes` |
| `body_md` | `text` | NOT NULL | 初期 Markdown |
| `required_fields_json` | `jsonb` | NOT NULL | 必須フィールド定義 |
| `is_active` | `boolean` | NOT NULL | 利用可否 |
| `created_at` | `timestamptz` | NOT NULL | 作成時刻 |

### 5.7 添付 / 下書き / ブックマーク / 議事録アクション

#### `attachments`

| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `attachment_id` | `uuid` | PK | 添付 ID |
| `target_type` | `text` | NOT NULL | `article` / `comment` |
| `target_id` | `uuid` | NOT NULL | article_id or comment_id |
| `file_name` | `text` | NOT NULL | 表示名 |
| `mime_type` | `text` | NOT NULL | MIME type |
| `file_size` | `bigint` | NOT NULL | サイズ byte |
| `storage_key` | `text` | NOT NULL | オブジェクトストレージキー |
| `uploaded_by_user_id` | `uuid` | FK users | アップロード者 |
| `created_at` | `timestamptz` | NOT NULL | 作成時刻 |
...
| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `draft_revision_id` | `uuid` | PK | revision ID |
| `draft_key` | `uuid` | NOT NULL | 下書き識別子 |
| `user_id` | `uuid` | FK users | 所有者 |
| `article_id` | `uuid` | FK articles, NULL | 既存記事下書きの場合 |
| `title` | `text` | NOT NULL | タイトル |
| `body_md` | `text` | NOT NULL | 本文 |
| `metadata_json` | `jsonb` | NOT NULL | category/tag/visibility など |
| `conflict_base_revision_number` | `integer` | NULL | 競合判定用基準 |
| `created_at` | `timestamptz` | NOT NULL | 作成時刻 |

ユニーク制約:

- `(draft_key, created_at)`

#### `draft_archive_events`

| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `draft_archive_event_id` | `uuid` | PK | event ID |
| `draft_key` | `uuid` | NOT NULL | 対象下書き |
| `user_id` | `uuid` | FK users | 実行者 |
| `occurred_at` | `timestamptz` | NOT NULL | 削除時刻 |

#### `article_stock_events`

| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `article_stock_event_id` | `uuid` | PK | event ID |
| `article_id` | `uuid` | FK articles | 対象記事 |
| `user_id` | `uuid` | FK users | 実行ユーザー |
| `event_type` | `text` | NOT NULL | `stocked` / `unstocked` |
| `occurred_at` | `timestamptz` | NOT NULL | 発生時刻 |
...
| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `saved_filter_id` | `uuid` | PK | フィルター ID |
| `user_id` | `uuid` | FK users | 所有者 |
| `name` | `text` | NOT NULL | フィルター名 |
| `conditions_json` | `jsonb` | NOT NULL | author/tag/stocked/commented など |
| `is_default` | `boolean` | NOT NULL | 初期表示に使うか |
| `created_at` | `timestamptz` | NOT NULL | 作成時刻 |
...
| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `action_item_id` | `uuid` | PK | Action ID |
| `article_id` | `uuid` | FK articles | 議事録記事 |
| `description` | `text` | NOT NULL | タスク内容 |
| `assignee_user_id` | `uuid` | FK users, NULL | 担当者 |
| `due_date` | `date` | NULL | 期限 |
| `status` | `text` | NOT NULL | `open` / `in_progress` / `done` / `canceled` |
| `created_by_user_id` | `uuid` | FK users | 作成者 |
| `created_at` | `timestamptz` | NOT NULL | 作成時刻 |

#### `meeting_action_item_events`

| カラム | 型 | 制約 | 説明 |
|---|---|---|---|
| `meeting_action_item_event_id` | `uuid` | PK | event ID |
| `action_item_id` | `uuid` | FK meeting_action_items | 対象 Action |
| `event_type` | `text` | NOT NULL | `status_changed` / `reassigned` / `edited` |
| `payload_json` | `jsonb` | NOT NULL | 差分 |
| `actor_user_id` | `uuid` | FK users | 実行者 |
| `occurred_at` | `timestamptz` | NOT NULL | 発生時刻 |
...
### 6.4 collaboration / template / draft

| view | 内容 |
|---|---|
| `v_current_spaces` | 各スペースの最新属性 |
| `v_active_space_memberships` | 最新イベントが有効なスペース所属関係 |
| `v_current_article_paths` | 各記事の最新 space/path |
| `v_active_article_path_redirects` | 旧パスから current article_id への参照 |
| `v_current_drafts` | archive されていないユーザー下書き最新状態 |
| `v_current_stock_state` | ユーザーごとの現在 Stock 状態 |
| `v_meeting_action_item_state` | 議事録 Action Item の現在状態 |

### 6.5 notification / search
...
### 7.6 階層パス current state

`article_path_revisions` の最大 `revision_number` を current path とする。`article_path_redirects` は旧パス解決用に参照し、最終的に current article へ正規化する。

### 7.7 下書き current state

`draft_key` ごとに最新 `draft_revisions` を取り、`draft_archive_events` が存在しないものだけを有効下書きとする。

### 7.8 Stock current state

`article_id + user_id` ごとに最新 `article_stock_events` を取り、`event_type = 'stocked'` のものだけを current stock とする。

### 7.9 議事録 Action Item current state

`meeting_action_items` を anchor とし、`meeting_action_item_events` を時系列適用して現在の担当者・状態を導出する。
...
- `article_path_revisions(article_id, revision_number desc)`
- `article_path_revisions(space_id, path, revision_number desc)`
- `attachments(target_type, target_id, created_at desc)`
- `draft_revisions(user_id, draft_key, created_at desc)`
- `article_stock_events(article_id, user_id, occurred_at desc)`
- `saved_filters(user_id, created_at desc)`
- `meeting_action_items(article_id, created_at desc)`
...
- `article_path_revisions` の `(space_id, path)` は current state で一意でなければならない
- `attachments.target_type='article'` の場合 `target_id` は `articles.article_id` を参照し、`comment` の場合は `comments.comment_id` を参照する（アプリまたはトリガーで保証）
- `saved_filters` はユーザーごとに `is_default=true` を 1 件までとする
- `meeting_action_items` は議事録用途テンプレートの記事にのみ作成可能とする
